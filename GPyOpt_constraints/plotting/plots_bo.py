...
def plot_acquisition(bounds, input_dim, model, Xdata, Ydata, acquisition_function, suggested_sample,
    ...
    if input_dim ==1:
        ...
        x_grid = np.linspace(bounds[0][0], bounds[0][1], 100*100) # change from size 200 to size 100 for consistency
    ...
    if input_dim == 2:
        ...
        X1 = np.linspace(bounds[0][0], bounds[0][1], 100) # change from size 200 to size 100 for consistency
        X2 = np.linspace(bounds[1][0], bounds[1][1], 100) # change from size 200 to size 100 for consistency
        x1, x2 = np.meshgrid(X1, X2)
        X = np.hstack((x1.reshape(100*100,1),x2.reshape(100*100,1))) # change from size 200 to size 100 for consistency
        acqu = acquisition_function(X)
        acqu_normalized = (-acqu - min(-acqu))/(max(-acqu - min(-acqu)))
        acqu_normalized = acqu_normalized.reshape((100,100))
        m, v = model.predict(X)
        plt.figure(figsize=(15,5))
        plt.subplot(1, 3, 1)
        plt.contourf(X1, X2, m.reshape(100,100),100)
        plt.colorbar()
        if color_by_step:
        ...
        ##
        plt.subplot(1, 3, 2)
        plt.contourf(X1, X2, np.sqrt(v.reshape(100,100)),100)
        plt.colorbar()
        if color_by_step:
            points_var_color(Xdata)
        else:
        ...
        if filename!=None:
            ...
            plt.show()
        return acqu_normalized # Aleks edit
